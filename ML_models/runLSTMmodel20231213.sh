...
#SBATCH --reservation=GPUTest          # reserve the GPU


#SBATCH --begin=2023-10-20T09:00:00# job start time, if it later than NOW, job will be run immediatly.

#SBATCH --output /home/qizhou/3paper/2AGU/2LSTM/logs/out_%A_%a_%x.txt         # Standard Output Log File (for Job Arrays)
#SBATCH --error  /home/qizhou/3paper/2AGU/2LSTM/logs/err_%A_%a_%x.txt         # Standard Error Log File (for Job Arrays)
...
srun --gres=gpu:A100:1 python /home/qizhou/3paper/2AGU/2LSTM/2RNN_LSTM20231213.py --input_station  "$current_parameter1" --bl_or_rf "$current_parameter2" --sequence_length "$current_parameter3" --batch_size 16
