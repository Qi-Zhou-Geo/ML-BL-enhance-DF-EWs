...
# <editor-fold desc="Transformer NOT finished">
...
class TimeSeriesTransformer(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, num_heads, output_size, max_len=512):
    ...
    def forward(self, x):
        x = self.embedding(x)
        x = x.permute(1, 0, 2)  # Change the order for the transformer input
...
def runType1(featureSIZE):

    # <editor-fold desc="load data as df">
    if featureTYPE == "bl_rf":
        df = load_bl_rf_Features(STATION)
    elif featureTYPE == "bl" or featureTYPE == "rf":
        df = loadSingleTypeFeatures(STATION, featureTYPE)
    elif featureTYPE == "4features":
        df = load_4Features(STATION)
    ...
    # train the model
    trainer = LSTM_Trainer(train_model, optimizer, train_dataloader.dataLoader())
    ...
    tester = LSTM_Tester(test_model, test_dataloader.dataLoader())
    tester.testing_step()

    print("finished testing", datetime.now().strftime("%Y-%m-%d %H:%M:%S"), flush=True)

def runType2(featureSIZE, scalerData):
    scaler = MinMaxScaler()
    ...
    # <editor-fold desc="load 2013-2014 test data">
    test_model = LstmClassifier(feature_size=featureSIZE, dropout=0.25)
    ...
    if featureTYPE == "bl_rf":
        df2013_2014 = load_bl_rf_Features(stationMapping.get(STATION), "BHZ", "2013-2014")
    elif featureTYPE == "bl" or featureTYPE == "rf":
        df2013_2014 = loadSingleTypeFeatures(stationMapping.get(STATION), featureTYPE, "BHZ", "2013-2014")
    elif featureTYPE == "4features":
        df2013_2014 = load_4Features(stationMapping.get(STATION), "BHZ", "2013-2014")
    else:
        print("check the featureTYPE")

    test_2013, test_2014 = splitDataFrame(df2013_2014, splitID=102241)
    if scalerData == True:
        test_2013.iloc[:, :-3] = scaler.transform(test_2013.iloc[:, :-3])
        test_2014.iloc[:, :-3] = scaler.transform(test_2014.iloc[:, :-3])

    test_2013_sequences = DataToSeq(df=test_2013, seq_length=sequenceLEN)
    test_2013_dataloader = DatasetToDataloader(data_sequences=test_2013_sequences, batch_size=batchSIZE, training_or_testing="testing")
    tester2013 = LSTM_Tester(test_model, test_2013_dataloader.dataLoader())
    tester2013.testing_step()


    test_2014_sequences = DataToSeq(df=test_2014, seq_length=sequenceLEN)
    test_2014_dataloader = DatasetToDataloader(data_sequences=test_2014_sequences, batch_size=batchSIZE, training_or_testing="testing")
    tester2014 = LSTM_Tester(test_model, test_2014_dataloader.dataLoader())
    tester2014.testing_step()
    # </editor-fold>
...
def main(input_station:str, bl_or_rf:str, sequence_length:int, batch_size:int):
    ...
    OUTPUT_DIR = f"/home/qizhou/3paper/2AGU/2LSTM/LSTM{featureTYPE}/"
    OUTPUT_NAME = f"{STATION}_LSTM_s{sequenceLEN}b{batchSIZE}"
    ...
    if featureTYPE == "bl":
        ...
        featureSIZE = 69
    elif featureTYPE == "bl_rf":
        featureSIZE = 80
    elif featureTYPE == "4features":
        featureSIZE = 6

    #runType1(featureSIZE)
    runType2(featureSIZE, scalerData=True)
    #runType3(featureSIZE)
...
if __name__ == "__main__":
    ...
    main(args.input_station, args.bl_or_rf, args.sequence_length, args.batch_size)
