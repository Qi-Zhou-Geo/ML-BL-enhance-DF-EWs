...
def runCal_loop (year, id1, id2, component):
    ...
    with open(f"{OUTPUT_DIR}{year}_{component}net.txt", 'a') as file:
    ...
    for julday in range(id1, id2):  # 91 = 1st of May to 305=31 of Nov.
        ...
        np_dir = f"/home/qizhou/1projects/dataForML/out60/{year}/{station_l[0]}/{component}/"# set np_dir
        dataILL18 = np.load(f"{np_dir}{year}_{station_l[0]}_{component}_{julday}.npy")  # 3identifiers + rms + 12001data

        np_dir = f"/home/qizhou/1projects/dataForML/out60/{year}/{station_l[1]}/{component}/"# set np_dir
        dataILL12 = np.load(f"{np_dir}{year}_{station_l[1]}_{component}_{julday}.npy")  # 3identifiers + rms + 12001data

        np_dir = f"/home/qizhou/1projects/dataForML/out60/{year}/{station_l[2]}/{component}/"# set np_dir
        dataILL13 = np.load(f"{np_dir}{year}_{station_l[2]}_{component}_{julday}.npy")  # 3identifiers + rms + 12001data

        ####################################

        # check the data length
        if len(dataILL18) != len(dataILL12) or len(dataILL18) != len(dataILL13) or len(dataILL12) != len(dataILL13):
            print("check the input data length: ", year, julday)
            print(len(dataILL18))
            print(len(dataILL12))
            print(len(dataILL13))
            exit()
...
def main(year, input_component):  # Update the global variables with the values from command-line arguments
    print(f"Start Job: {year}, {input_component}: ", datetime.now().strftime("%Y-%m-%d %H:%M:%S") )

    global SPS, OUTPUT_DIR, SAC_DIR
    OUTPUT_DIR = "/home/qizhou/1projects/dataForML/out60/" + str(year) + "/"
    ...
    runCal_loop(year, id1, id2, input_component)  # run the loop


    print(f"End Job: {year}, {input_component}: ", datetime.now().strftime("%Y-%m-%d %H:%M:%S") )

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--year", type=int, default=2020, help="cal_year")
    parser.add_argument("--input_component", type=str, default="ILL12", help="check the input_station")

    args = parser.parse_args()
    main(args.year, args.input_component)
